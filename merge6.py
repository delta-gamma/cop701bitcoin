...
class Peer:   
    ...
    def __init__(self):
        ...
        self.clientport = random.randint(49152, 65535)
        self.print_ledger()
        ...
        self.nodeid = 1000+int(self.ind) #nodeid=100x
        ...
        print 'Server port of', self.nodeid, 'is ', self.serverport
        print 'Client port of', self.nodeid, 'is ', self.clientport
            ...
            if(dist!=0):
                ...
                if(j<d-1):
                    ...
                    if((d-math.floor(d)==0 and d<=(2**(hash_bit-1)))):
                        j+=1
                        self.ft[nid] = '10.0.0.'+str(index)
        print 'Finger table built for ', self.nodeid, self.ft
 
    def randomoffon(self): #a node goes offline or online with random probability
        global nodes
        time.sleep(30)
        while True:
            time.sleep(0.2)
            ...
            if self.state==0:
                randon=10
                if(randon==random.randint(1, 10000)):
    ...
    def find_pk_rec(self, xip, nid):#when a node recursively search for sender IP on other nodes
        #print 'find pk rec', xip, nid, self.ft
        b=0
        for key in sorted(self.ft, reverse=True):
#            print 'key searched rec: ', key
            if int(key)==int(nid):
                b=1
                #print "matched rec"
                index=int(nid)-1000
    ...
    def pk_recvd(self, pk): #node has received the pk of sender
        print 'pk recvd'
        #print 'received pk is: ', pk
        self.newpk= RSA.importKey(pk)
        #print self.newpk
...
#----------------UDP CODES--------------------------------------------------------------------------->
    ...
    def select_function(self, temp):
        ...
        elif(flag=="udpu"): #when a node comes online, other nodes send the number of online nodes in their list
...
#----------------TCP CODES--------------------------------------------------------------------------->	    
    ...
    def tcpclienthandler(self, msg):
        ...
        s.send(str(l[2])+' '+str(l[5])) #sending the amount to recv/witness
        #time.sleep(2)
        x=None
        while(x==None):
            x=s.recv(1024)
        ...
        if x=='commit':
            if flag=='recv':
                self.crecvflag=True
                #print 'witflag ',self.cwitflag
                #print 'Approved by reciever'
            if flag=='wit':
                self.cwitflag=True
                #print 'recvflag ',self.crecvflag
                #print 'Approved by witness'	
    ...
    def tcpclientcode(self, msg):
        ...
        #self.udpclientcode(trans_id, self.nodeid, recvid, amount)        
...
#----------------- LEDGER ---------------------------------------------------------------------->   
    ...
    def update_ledger(self, container):
    	...
    	if(self.nodeid == receiver):
    		self.bitcoins = self.bitcoins + amount
    	print trans_id, sender, receiver, amount
...
#----------------------when node wants to send bitcoins-----------------------------#            
    ...
    def trans1(self):
	    if (self.nodeid==1002):
		    recvid = 1004 #recv nodid
		    sendTo = '10.0.0.4'      #recv ip
		    self.sender_trans(sendTo, recvid, '1000')
    
    def trans2(self):
	    if (self.nodeid==1001):
		    recvid = 1003 #recv nodid
		    sendTo = '10.0.0.3'      #recv ip
		    self.sender_trans(sendTo, recvid, '1000')
...
if __name__ == '__main__':
    ...
    if num_nodes==0:
        ...
        peerobj.create_ft()
        time.sleep(5)
    p5=threading.Thread(target=peerobj.trans1) #thread for tcp server
    #p5.daemon = True

    #time.sleep(5)
    p6=threading.Thread(target=peerobj.trans2) #thread for tcp server
    #p6.daemon = True
    p5.start()
    p6.start()
    ...
    while True:
        opt = raw_input("Please choose:\nPress 2: Send Bitcoins -> \nPress 3: Verify Ledger ->")
        if(opt=='1'):
            continue                    
        elif(opt=='2'):
            if peerobj.state==1:
                sendTo = raw_input("Whom do you want to send? ")    #recv h1..hn
                amount = raw_input("Amount of money to send? ")
                if int(sendTo[1:]) in nodes:
                    recvid = 1000+int(sendTo[1:]) #recv nodid
                    sendTo = '10.0.0.'+sendTo[1:]      #recv ip
